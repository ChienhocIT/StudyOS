...
export function Providers({ children }: { children: ReactNode }) {
          ...
          try {
            ...
            if (previous)
              sessionStorage.setItem(
                SESSION_KEY,
                JSON.stringify({ ...previous, tokens }),
              );
          } catch {
            sessionStorage.removeItem(SESSION_KEY);
          }
  ...
  useEffect(() => {
    try {
      ...
      if (saved?.tokens?.refreshToken && saved?.user?.id) {
        api.setTokens(saved.tokens);
        setUser(saved.user);
      }
    } catch {
      sessionStorage.removeItem(SESSION_KEY);
  ...
  const authenticate: AuthContextValue["authenticate"] = async (mode, body) => {
    const result = await api.post<Entity<"AuthResponse">>(
      `/api/v1/auth/${mode}`,
      body,
    );
    queryClient.clear();
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <AuthContext.Provider value={{ api, user, ready, authenticate, logout }}>
        {children}
