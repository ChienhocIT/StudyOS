...
const tabs = [
  { id: "chat", label: "Hỏi tài liệu", icon: MessageSquare },
  { id: "notes", label: "Ghi chú", icon: PencilLine },
  { id: "studio", label: "Góc luyện tập", icon: Sparkles },
  { id: "review", label: "Thẻ ghi nhớ", icon: Layers3 },
  { id: "learning", label: "Kiến thức", icon: Target },
  { id: "language", label: "Ngôn ngữ", icon: Languages },
  { id: "analytics", label: "Tiến độ", icon: ChartNoAxesCombined },
] as const;
export function NotebookWorkspace({
  ...
  notebook: Notebook;
  onChanged: (notebook: Notebook) => void;
}) {
  const { api } = useAuth();
  ...
  const sources = useResource<Source[]>(path, (data) =>
    data?.some(
      (source) => !["READY", "FAILED", "DELETED"].includes(source.status),
    )
      ? 2500
      : false,
  );
  const save = useAction(
    (body: { title: string; description: string; goalText: string }) =>
      api.patch<Notebook>(`/api/v1/notebooks/${notebook.id}`, body),
    [`/api/v1/workspaces/${notebook.workspaceId}/notebooks`],
  );
  function navigateCitation(value: StreamCitation) {
    setCitation(value);
  ...
  return (
    <>
      <div className="notebook-heading">
        <div className="row">
          <span className="notebook-title-icon">
            <BookOpen size={23} />
          </span>
          <div>
            ...
            <p className="muted">
              {notebook.goalText ||
                ...
                "Thêm tài liệu, đặt câu hỏi và xây dựng kiến thức của bạn."}
            </p>
          </div>
        </div>
        <button
      ...
      <div className="notebook-tabs" role="tablist" aria-label="Công cụ sổ tay">
        {tabs.map((item) => (
          <button
            key={item.id}
            role="tab"
      ...
      <div
        className={`notebook-layout ${sourceVisible ? "sources-visible" : ""}`}
      >
        <aside className="sources-column">
          <SourcePanel
            ...
            onSelection={setSelectedSources}
            citation={citation}
        ...
        >
          <button
            className="button show-sources"
            onClick={() => setSourceVisible(true)}
          >
            <FileText size={16} />
            Tài liệu nguồn ({sources.data?.length || 0})
          </button>
          {tab === "chat" && (
            <ChatPanel
              notebookId={notebook.id}
              sourceIds={selectedSources.filter((id) =>
                sources.data?.some(
          ...
          {tab === "notes" && <NotesPanel notebookId={notebook.id} />}
          {tab === "studio" && (
            <StudioPanel
              notebookId={notebook.id}
          ...
          {tab === "analytics" && <AnalyticsPanel notebookId={notebook.id} />}
        </section>
      </div>
      {edit && (
        <Modal
          ...
          onClose={() => {
            if (!save.isPending) setEdit(false);
          }}
        >
          <form
            className="stack"
              ...
              save.mutate(
                {
                  ...
                  goalText: String(data.get("goalText")).trim(),
                },
                {
                  onSuccess: (value) => {
                    onChanged(value);
          ...
          >
            ...
            <label>
              ...
              <textarea
                name="goalText"
                defaultValue={notebook.goalText || ""}
              />
            </label>
