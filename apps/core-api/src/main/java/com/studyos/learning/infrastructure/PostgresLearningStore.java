...
public class PostgresLearningStore implements LearningStore {
    ...
    public PostgresLearningStore(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public boolean conceptInNotebook(UUID conceptId, UUID notebookId) {
        return Boolean.TRUE.equals(jdbc.queryForObject(
                ...
                Boolean.class, conceptId, notebookId));
    }

    public Map<String, Object> appendEvidence(UUID userId, UUID conceptId, String type,
            BigDecimal score, BigDecimal weight, String key, UUID activityId) {
        jdbc.update("INSERT INTO user_concept_mastery(user_id,concept_id) VALUES (?,?) ON CONFLICT DO NOTHING", userId, conceptId);
        jdbc.queryForObject("SELECT version FROM user_concept_mastery WHERE user_id=? AND concept_id=? FOR UPDATE",
        ...
        int inserted = jdbc.update("""
                ...
                """, userId, conceptId, type, score, weight, key, Json.write(Map.of("activityId", activityId, "policy", "mastery-v0")));
        ...
        return jdbc.queryForObject("""
                ...
                """, Rows::map, userId, conceptId);
    }

    public void updateProjection(UUID userId, UUID conceptId, BigDecimal score, BigDecimal confidence) {
        jdbc.update("""
                ...
                """, score, confidence, userId, conceptId, userId, conceptId);
    }
    ...
    public List<Map<String, Object>> mastery(UUID userId, UUID notebookId) {
        return jdbc.query("""
                ...
                """, Rows::map, userId, notebookId);
    }

    public void acceptConcept(UUID workspaceId, UUID notebookId, UUID sourceId, String name,
            String normalizedName, String description, BigDecimal confidence, List<UUID> chunks) {
        ...
        for (UUID chunk : chunks) {
            boolean valid = Boolean.TRUE.equals(jdbc.queryForObject("""
                    ...
                    """, Boolean.class, chunk, workspaceId, notebookId, sourceId));
            if (!valid) throw ApiException.badRequest("CONCEPT_PROVENANCE_INVALID", "Concept evidence is outside the current source.");
        }
        UUID id = jdbc.queryForObject("""
                ...
                RETURNING id
                """, UUID.class, workspaceId, notebookId, name, normalizedName, description, confidence);
        for (UUID chunk : chunks) jdbc.update("""
    ...
    public Optional<Map<String, Object>> goal(UUID userId, UUID goalId) {
        return jdbc.query("SELECT * FROM learning_goals WHERE id=? AND user_id=?", Rows::map, goalId, userId).stream().findFirst();
    }

    public Map<String, Object> createGoal(UUID userId, UUID workspaceId, String title, String description,
            String targetDate, Integer weeklyMinutes) {
        return jdbc.queryForObject("""
                INSERT INTO learning_goals(user_id,workspace_id,title,description,target_date,weekly_minutes)
                VALUES (?,?,?,?,?::date,?) RETURNING *
                """, Rows::map, userId, workspaceId, title, description, targetDate, weeklyMinutes);
    }

    public Map<String, Object> updateGoal(UUID userId, UUID goalId, Map<String, Object> changes) {
        Map<String, String> allowed = Map.of("title", "title", "description", "description", "targetDate", "target_date",
                "weeklyMinutes", "weekly_minutes", "status", "status");
        for (var field : changes.entrySet()) {
            if (!allowed.containsKey(field.getKey())) throw ApiException.badRequest("VALIDATION_FAILED", "Unknown goal field.");
            String cast = field.getKey().equals("targetDate") ? "::date" : "";
            jdbc.update("UPDATE learning_goals SET " + allowed.get(field.getKey()) + "=?" + cast
                    + ",updated_at=now() WHERE id=? AND user_id=?", field.getValue(), goalId, userId);
        }
        return goal(userId, goalId).orElseThrow();
    }

    public List<Map<String, Object>> recommendations(UUID userId) {
        // Serialize snapshot replacement for one learner; no recommendations survive a membership revocation.
        jdbc.queryForObject("SELECT id FROM users WHERE id=? FOR UPDATE", UUID.class, userId);
        jdbc.update("UPDATE learning_recommendations SET status='EXPIRED' WHERE user_id=? AND status='PENDING'", userId);
        List<Map<String, Object>> candidates = jdbc.query("""
                ...
                """, Rows::map, userId, userId, userId, userId);
        ...
        for (var candidate : candidates) {
            ...
            if (due > 0 && reviewNotebooks.add(notebookId)) {
                insertRecommendation(userId, notebookId, null, "REVIEW_FLASHCARDS", new BigDecimal("1.0"),
                        Map.of("policy", "recommendation-v0", "dueCards", due, "message", "Ôn thẻ đến hạn trước khi học thêm."));
            }
            ...
            BigDecimal priority = BigDecimal.ONE.subtract(mastery).multiply(new BigDecimal("0.7"))
                    .add(activeGoal ? new BigDecimal("0.15") : BigDecimal.ZERO);
            insertRecommendation(userId, notebookId, Rows.uuid(candidate, "conceptId"), mastery.signum() == 0 ? "LEARN_NEW" : "RETRY_QUIZ",
                    priority, Map.of("policy", "recommendation-v0", "masteryGap", BigDecimal.ONE.subtract(mastery),
                            "goalAligned", activeGoal, "conceptName", candidate.get("name")));
        }
        return jdbc.query("""
    ...
    private void insertRecommendation(UUID userId, UUID notebookId, UUID conceptId, String action, BigDecimal priority, Object reason) {
        jdbc.update("""
                ...
                """, userId, notebookId, conceptId, action, priority, Json.write(reason));
    }
}
