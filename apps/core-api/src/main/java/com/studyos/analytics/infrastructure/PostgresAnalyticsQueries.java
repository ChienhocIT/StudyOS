    ...
    public Map<String, Object> overview(UUID user, UUID notebook) {
        return jdbc.queryForObject(
                ...
                WITH accessible AS (
                  ...
                  (SELECT count(*) FROM flashcards f JOIN flashcard_decks d ON d.id=f.deck_id AND d.status='READY' JOIN notebooks n ON n.id=d.notebook_id AND n.status='ACTIVE' WHERE d.user_id=? AND d.notebook_id IN(SELECT id FROM accessible) AND coalesce(f.due_at,f.created_at)<=now()) AS due_cards,
