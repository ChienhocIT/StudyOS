...
public class PostgresReviewStore implements ReviewStore {
    ...
    public PostgresReviewStore(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<Map<String, Object>> queue(UUID user, UUID notebook, int limit) {
        return jdbc.query(
                ...
                FROM flashcards f JOIN flashcard_decks d ON d.id=f.deck_id AND d.status='READY'
    ...
    public Optional<Map<String, Object>> card(UUID user, UUID card, boolean lock) {
        return jdbc
                .query(
                        """
                SELECT f.*,d.notebook_id FROM flashcards f JOIN flashcard_decks d ON d.id=f.deck_id AND d.status='READY'
    ...
    public Optional<Map<String, Object>> priorReview(UUID user, String key) {
        return jdbc
                .query(
                        "SELECT * FROM flashcard_reviews WHERE user_id=? AND idempotency_key=?",
                        Rows::map,
                        user,
                        key)
                .stream()
    ...
    public Instant lastReviewedAt(UUID user, UUID card) {
        Timestamp value =
                jdbc.queryForObject(
                        "SELECT max(reviewed_at) FROM flashcard_reviews WHERE user_id=? AND card_id=?",
                        Timestamp.class,
                        user,
                        card);
    ...
    public Map<String, Object> saveReview(
        ...
        jdbc.update(
                "UPDATE flashcards SET scheduler_state_json=?::jsonb,due_at=?,updated_at=? WHERE id=?",
                Json.write(newState),
                Timestamp.from(dueAt),
                Timestamp.from(now),
                card);
    ...
    public UUID vocabularyCard(
            UUID user, UUID notebook, UUID vocabulary, String front, String back) {
        UUID deck =
                UUID.nameUUIDFromBytes(
                        ("vocabulary-deck:" + user + ":" + notebook)
                                .getBytes(StandardCharsets.UTF_8));
        UUID card =
                UUID.nameUUIDFromBytes(
                        ("vocabulary-card:" + vocabulary).getBytes(StandardCharsets.UTF_8));
        jdbc.update(
                "INSERT INTO flashcard_decks(id,notebook_id,user_id,title) VALUES (?,?,?,'Từ vựng đã lưu') ON CONFLICT DO NOTHING",
                deck,
                notebook,
                user);
        jdbc.update(
                "INSERT INTO flashcards(id,deck_id,front,back,due_at) VALUES (?,?,?,?,now()) ON CONFLICT DO NOTHING",
                card,
                deck,
                front,
                back);
