...
import com.studyos.note.application.NoteService;
...
public class NoteController {
    ...
    public record Create(
            ...
            @Size(max = 100) List<@NotNull UUID> citationIds) {}

    public record Patch(
            @Pattern(regexp = ".*\\S.*", flags = Pattern.Flag.DOTALL) @Size(max = 240) String title,
    ...
    public Map<String, Object> create(@PathVariable UUID notebookId, @Valid @RequestBody Create r) {
        return service.create(
                Actor.currentUserId(), notebookId, r.title(), r.content(), r.citationIds());
    }
    ...
    public Map<String, Object> update(@PathVariable UUID id, @Valid @RequestBody Patch r) {
        return service.update(Actor.currentUserId(), id, r.title(), r.content(), r.citationIds());
    }
