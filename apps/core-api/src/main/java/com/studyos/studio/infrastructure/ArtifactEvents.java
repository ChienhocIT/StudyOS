...
public class ArtifactEvents {
    ...
    public ArtifactEvents(StudioService service) {
        this.service = service;
    }

    @Bean
    Declarables artifactStateQueue() {
        var queue =
                QueueBuilder.durable("q.core.artifact-state.v1")
                        .deadLetterExchange("studyos.dlx")
                        .build();
        List<Declarable> definitions = new ArrayList<>();
    ...
    @RabbitListener(queues = "q.core.artifact-state.v1")
    public void consume(Message message) {
        service.acceptEvent(Json.object(new String(message.getBody(), StandardCharsets.UTF_8)));
    }
}
