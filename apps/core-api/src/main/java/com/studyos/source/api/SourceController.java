...
public class SourceController {
    ...
    @GetMapping("/notebooks/{notebookId}/sources")
    public List<Map<String, Object>> list(@PathVariable UUID notebookId) {
        return service.list(Actor.currentUserId(), notebookId);
    }

    @PostMapping("/notebooks/{notebookId}/sources/raw")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public Map<String, Object> raw(
            @PathVariable UUID notebookId,
            @Valid @RequestBody Raw r,
            @RequestHeader(value = "Idempotency-Key", required = false) @Size(min = 1, max = 120)
                    String key) {
        return service.raw(Actor.currentUserId(), notebookId, r.title(), r.content(), key);
    }

    @PostMapping("/notebooks/{notebookId}/sources/url")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public Map<String, Object> url(
            @PathVariable UUID notebookId,
            @Valid @RequestBody Url r,
            @RequestHeader(value = "Idempotency-Key", required = false) @Size(min = 1, max = 120)
                    String key) {
        return service.url(Actor.currentUserId(), notebookId, r.url(), r.title(), key);
    }

    @PostMapping("/notebooks/{notebookId}/sources/upload-init")
    @ResponseStatus(HttpStatus.CREATED)
    public Map<String, Object> upload(
            @PathVariable UUID notebookId,
            @Valid @RequestBody Upload r,
            @RequestHeader(value = "Idempotency-Key", required = false) @Size(min = 1, max = 120)
                    String key) {
    ...
    @PostMapping("/notebooks/{notebookId}/sources/upload-complete")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public Map<String, Object> complete(
            @PathVariable UUID notebookId, @Valid @RequestBody Complete r) {
        return service.complete(
                Actor.currentUserId(), notebookId, r.sourceId(), r.checksumSha256(), r.sizeBytes());
    }

    @GetMapping({"/sources/{id}", "/sources/{id}/status"})
    public Map<String, Object> get(@PathVariable UUID id) {
        return service.get(Actor.currentUserId(), id);
    }

    @GetMapping("/sources/{id}/download")
    public Map<String, Object> download(@PathVariable UUID id) {
        return service.download(Actor.currentUserId(), id);
    }
    ...
    public void delete(@PathVariable UUID id) {
        service.delete(Actor.currentUserId(), id);
    }

    @GetMapping("/sources/{id}/transcript")
    public List<Map<String, Object>> transcript(@PathVariable UUID id) {
        return service.transcript(Actor.currentUserId(), id);
    }
}
